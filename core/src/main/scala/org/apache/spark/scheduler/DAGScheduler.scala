...
/**
  ...
  * 追踪RDD和Stage输出是否物化
  * 寻找最优调度机制
  * 将stage作为task sets，提交到TaskScheduler并在集群上运行tasks
  * 决定task的最佳位置，给予缓存状态，将最佳位置提交给TaskScheduler
  * 负责shuffle输出文件丢失导致的失败，可能重新提交旧的stage（Stage内部失败，不是由于shuffle，会被TaskScheduler处理）
  * 多次重试每一个task提交，不行的话取消整个stage
  */
...
class DAGScheduler(
          ...
          for (dep <- rdd.dependencies) {
            dep match {
              case shufDep: ShuffleDependency[_, _, _] =>
                // 宽依赖， 使用该rdd创建shuffleMapStage
                // 最后一个Stage已经存在，并且是finalStage，其余都新建ShuffleMapStage
                // 该rdd是新的stage的最后一个rdd
  ...
  // Stage 划分算法入口
  // 当一个Task完成后，onCompletion会调用submitWaitingChildStages，
  // 得到下一个对应ChildStage的Task
    ...
    if (jobId.isDefined) {
      ...
      if (!waitingStages(stage) && !runningStages(stage) && !failedStages(stage)) {
        ...
        if (missing.isEmpty) {
          ...
          // 没有父Stage
          // 提交Stage，为Stage创建Task
          submitMissingTasks(stage, jobId.get)
        } else {
          for (parent <- missing) {
            // 递归提交父Stage， 知道最初的Stage没有父Stage
            submitStage(parent)
          }
          // 加入等待队列
          waitingStages += stage
  ...
  private def submitMissingTasks(stage: Stage, jobId: Int) {
    ...
    if (tasks.size > 0) {
      ...
      // 针对stage的task，创建TaskSet对象，调用TaskScheduler.submit提交
  ...
  private def stageDependsOn(stage: Stage, target: Stage): Boolean = {
    ...
    def visit(rdd: RDD[_]) {
      if (!visitedRdds(rdd)) {
        ...
        for (dep <- rdd.dependencies) {
          dep match {
            // TODO Where? 对于shuffle操作，底层对应三个RDD， MapPartitionRDD, ShuffleRDD, MapPartitionedRDD
            case shufDep: ShuffleDependency[_, _, _] =>
              val mapStage = getOrCreateShuffleMapStage(shufDep, stage.firstJobId)
              if (!mapStage.isAvailable) {
  ...
  // 从stage最后一个rdd开始寻找哪个rdd的partition是被cache或checkpoint了
  // task在那个节点执行不需要计算之前的RDD
